...
public class OrderService {
    ...
    public Order createOrder(Customer customer, DeliveryService deliveryService){
        ...
        return orderDAO.createOrder(order)
                .orElseThrow(() -> new EntityCreationException(ORDER_CREATION_EXCEPTION.formatted(orderCode)));
    }
    ...
    private Order getOrder(String orderCode){
        return orderDAO.findOrderByOrderCode(orderCode)
                .orElseThrow(() -> new EntityReadException(ORDER_READ_EXCEPTION.formatted(orderCode)));
    }
